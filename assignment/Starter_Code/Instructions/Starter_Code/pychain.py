...
@dataclass
class Block:
    record: Record  # Renamed 'data' attribute to 'record' with data type 'Record'
    creator_id: int
    prev_hash: str = "0"
...
def add_transaction_record():
    ...
    if st.button("Add Record"):
        pychain.add_block(Block(Record(sender, receiver, amount), pychain.chain[-1].creator_id + 1))
...
def add_transaction_record():
    ...
    if st.button("Add Record"):
        pychain.add_record(sender, receiver, amount)
